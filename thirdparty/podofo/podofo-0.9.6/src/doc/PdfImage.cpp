...
void PdfImage::LoadFromFile( const char* pszFilename )
{
    if( pszFilename && strlen( pszFilename ) > 3 )
    {
        ...
        if( PoDoFo::compat::strncasecmp( pszExtension, "tif", 3 ) == 0 ||
            PoDoFo::compat::strncasecmp( pszExtension, "TIF", 3 ) == 0 ||
            PoDoFo::compat::strncasecmp( pszExtension, "tiff", 4 ) == 0 ||
        ...
        if( PoDoFo::compat::strncasecmp( pszExtension, "jpg", 3 ) == 0 || 
            PoDoFo::compat::strncasecmp( pszExtension, "jpeg", 4 ) == 0 )
        {
...
void PdfImage::LoadFromPng( const char* pszFilename )
{
	...
	png_get_IHDR(pPng, pInfo,&width, &height, &depth,&color_type, &interlace, NULL, NULL);
	// End //
	...
	long pngBufLen = static_cast<long>(rowbytes * height);
	char* pBuffer = static_cast<char*>(malloc(sizeof(char) * pngBufLen));
	if (pBuffer)
	{
		...
		{
			{
				...
				{
					pRows[y] = reinterpret_cast<png_bytep>(pBuffer + (y * rowbytes));
				}
			...
			{
				png_bytep *row_ptr, one_row;
				row_ptr = pRows;
				char* smask_data = static_cast<char*>(malloc(sizeof(char) * imgsize));
				if (smask_data)
				{
					...
					for (int y = 0; y < height; y++)
					{
						...
						{
							smask_data[width * y + x] = one_row[4 * x + 3];
						}
					}
					...
					PdfMemoryInputStream stream(pBuffer, imgsize * 3);
					this->SetImageData(width, height, depth, &stream);

					{
						PdfMemoryInputStream smaskStream(smask_data, imgsize);

						PdfImage smaskImage(this->GetObject()->GetOwner());
						smaskImage.SetImageColorSpace(ePdfColorSpace_DeviceGray);
						smaskImage.SetImageData(width, height, depth, &smaskStream);
						//set the smask to this image

						this->SetImageSoftmask(&smaskImage);
					}
			...
			else
			{
				...
				// Set the image data and flate compress it
				PdfMemoryInputStream stream(pBuffer, pngBufLen);
				this->SetImageData(width, height, depth, &stream);
			}
